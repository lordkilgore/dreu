...
$$
In our experiments, we take $\mathcal{X} = \{\text{SHAPE}, \text{ANGLE}\}$ and use this methodology to compute the $\text{effect}[\text{ANGLE}][\text{layer}]$ and $\text{effect}[\text{SHAPE}][\text{layer}]$ heatmaps for the shape and angle variables respectively.

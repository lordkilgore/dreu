...
If it is truly the case that the geodesics are linear across both the arithmetic and shape modalities, it would be pretty surprising. By the chain rule, this would suggest that reproducing the results of Oozeer et al. (2026) with the pullback of the 28th layer from the output space would also lead to linear geodesics, not rounded ones. Verifying that the geodesics are linear with the weekday modality would confirm that my code is functionally incorrect.

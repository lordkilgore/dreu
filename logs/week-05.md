...
The geodesics deviate from the linear baseline and show sensitivity to carrier prompt in the induced traces in the output probability simplex. The induced traces are unnatural and the transitions are not smooth, but do often show a faster surfacing of the top-1 probability being the endpoint compared to the linear baseline. This is to be expected -- the pullback metric computes geodesics which induce, themselves, geodesics in the output probability simplex. These induced geodesics are not necessarily "natural", as seen in the work of Wurgaft et al, but arise as the result of the isometry of the rest-of-network map under the pullback metric. 
